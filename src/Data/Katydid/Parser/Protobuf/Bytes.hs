...
module Data.Katydid.Parser.Protobuf.Bytes
    ( getVarInt
    , putVarInt
    , anyBits
    , wordToFloat
...
getVarInt = loop 1 0
  where
    loop !s !n = do
        ...
        if (b .&. 128) == 0 then return $! n' else loop (128 * s) n'
...
-- | signedInt32ToWord encodes a signed int32 to a zig zag encoded varint
signedInt32ToWord :: Int32 -> Word32
signedInt32ToWord n = fromIntegral $ shiftL n 1 `xor` shiftR n 31

-- | wordToSignedInt32 decodes a zig zag encoded varint to a signed int32
wordToSignedInt32 :: Word32 -> Int32
wordToSignedInt32 n =
    fromIntegral (shiftR n 1) `xor` negate (fromIntegral $ n .&. 1)

-- | signedInt64ToWord encodes a signed int64 to a zig zag encoded varint
signedInt64ToWord :: Int64 -> Word64
signedInt64ToWord n = fromIntegral $ shiftL n 1 `xor` shiftR n 63

-- | wordToSignedInt64 decodes a zig zag encoded varint to a signed int64
