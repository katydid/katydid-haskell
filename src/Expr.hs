...
assertArgs1 _ [e1] = return e1
assertArgs1 exprName es = throwError $ exprName ++ ": expected one argument, but got " ++ show (length es) ++ ": " ++ show es
...
assertArgs2 _ [e1, e2] = return (e1, e2)
assertArgs2 exprName es = throwError $ exprName ++ ": expected two arguments, but got " ++ show (length es) ++ ": " ++ show es
