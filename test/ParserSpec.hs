...
success
  :: (Eq a, Show a) => String -> CharParser () a -> String -> a -> T.TestTree
success name p input want =
  HUnit.testCase name $ case parse (p <* eof) "" input of
    (Left err) ->
      HUnit.assertFailure
        $  "given input: "
        ++ input
        ...
        ++ show err
    (Right got) ->
      HUnit.assertEqual ("want: " ++ show want ++ " got: " ++ show got) want got
...
tests = T.testGroup
  ...
  , success "bytes number with more spaces"
            bytesCastLit
            "[]byte{ 46 ,    46     , 46}"
            "..."
  ...
  , success "expr =="
            (expr mkExpr)
            "== true"
            (eqExpr varBoolExpr (boolExpr True))
  , success "expr *="
            (expr mkExpr)
            "*= \"a\""
            (containsStringExpr varStringExpr (stringExpr "a"))
  , success "expr not" (expr mkExpr) "not(true)" (notExpr (boolExpr True))
  , success "expr eq bool"
            (expr mkExpr)
            "eq($bool, true)"
            (eqExpr varBoolExpr (boolExpr True))
  , success "expr eq int"
            (expr mkExpr)
            "eq($int, 1)"
            (eqExpr varIntExpr (intExpr 1))
  , failure "expr eq type mismatch" (expr mkExpr) "eq($bool, 1)"
  , success
    "expr list"
    (expr mkExpr)
    "eq($int, length([]int{1,2}))"
    (eqExpr varIntExpr (lengthListExpr $ intsExpr [intExpr 1, intExpr 2]))
  ...
  , success "name string"
            nameExpr
            "\"a\""
            (eqExpr varStringExpr (stringExpr "a"))
  , success "name not"
            nameExpr
            "!(a)"
            (notExpr (eqExpr varStringExpr (stringExpr "a")))
  , success "name any" nameExpr "_" (boolExpr True)
  , success
    "name or"
    nameExpr
    "(a|b)"
    (orExpr (eqExpr varStringExpr (stringExpr "a"))
            (eqExpr varStringExpr (stringExpr "b"))
    )
  ...
  , success "or list longer"
            (pattern mkExpr)
            "(*|*|*|*|*)"
            (Or ZAny (Or (Or (Or ZAny ZAny) ZAny) ZAny))
  ...
  , success "concat list"
            (pattern mkExpr)
            "[*,*,*]"
            (Concat (Concat ZAny ZAny) ZAny)
  , success "interleave" (pattern mkExpr) "{*;*}" (Interleave ZAny ZAny)
  , success "interleave list"
            (pattern mkExpr)
            "{*;*;*}"
            (Interleave (Interleave ZAny ZAny) ZAny)
  ...
  , success "leaf builtin"
            (pattern mkExpr)
            "== 1"
            (Node (eqExpr varIntExpr (intExpr 1)) Empty)
  , success "leaf function"
            (pattern mkExpr)
            "->eq($int, 1)"
            (Node (eqExpr varIntExpr (intExpr 1)) Empty)
  , success "treenode"
            (pattern mkExpr)
            "a:*"
            (Node (eqExpr varStringExpr (stringExpr "a")) ZAny)
  , success "any treenode" (pattern mkExpr) "_:*" (Node (boolExpr True) ZAny)
  , success "treenode no colon"
            (pattern mkExpr)
            "_[*,*]"
            (Node (boolExpr True) (Concat ZAny ZAny))
  , success
    "treenode with contains"
    (pattern mkExpr)
    "a:*=\"b\""
  ...
  , success
    "contains anynode with contains"
    (pattern mkExpr)
    "._:*=\"b\""
    (Contains $ Node (boolExpr True) $ Node
      (containsStringExpr varStringExpr (stringExpr "b"))
      Empty
    )
  , success
    "contains anynode with contains or"
    (pattern mkExpr)
    "(._:*=\"b\"|*)"
    (Or
      (Contains $ Node (boolExpr True) $ Node
  ...
  ,
    -- (~=\"^([ \t\r\n\v\f])+$\")*
    success
    "Page195E0AddrE0NameE0"
    (pattern mkExpr)
    "Person:{Name:*;(Addr:*)?;(Email:*)*}"
    (Node
      (eqExpr varStringExpr (stringExpr "Person"))
      (Interleave
        (Interleave
  ...
  , success
    ...
    "(~=\"^([ \t\r\n\v\f])+$\")*"
    ( ZeroOrMore
    $ Node (regexExpr (stringExpr "^([ \t\r\n\v\f])+$") varStringExpr) Empty
    )
  , success
    "Page195E0AddrE0NameE0 with whitespace"
    (pattern mkExpr)
    "Person:{Name:*;(Addr:*)?;(Email:*)*;(~=\"^([ \t\r\n\v\f])+$\")*}"
    (Node
      (eqExpr varStringExpr (stringExpr "Person"))
      (Interleave
        ...
        (ZeroOrMore $ Node
          (regexExpr (stringExpr "^([ \t\r\n\v\f])+$") varStringExpr)
          Empty
        )
      )
    )
  , success "single pattern grammar" (grammar mkExpr) "*" $ newRef "main" ZAny
  , success "single pattern decl" (grammar mkExpr) "#main = *"
    $ newRef "main" ZAny
  , failure "two patterns grammar" (grammar mkExpr) "* *"
  , success "two pattern decls" (grammar mkExpr) "#main = * #a = *"
  $       newRef "main" ZAny
  `union` newRef "a"    ZAny
  , success "one pattern and one pattern decl" (grammar mkExpr) "* #a = *"
  $       newRef "main" ZAny
  `union` newRef "a"    ZAny
  , success "one pattern and two pattern decls"
            (grammar mkExpr)
            "* #a = * #b = *"
  $       newRef "main" ZAny
  `union` newRef "a"    ZAny
  `union` newRef "b"    ZAny
  , success
    "not pattern, not name and != conflicts without not enough lookahead"
    (grammar mkExpr)
    "!(A):*"
    (newRef "main" (Node (notExpr (eqExpr varStringExpr (stringExpr "A"))) ZAny)
    )
  , success "->type conflicts with ->true and -1 conflicts with ->"
            (grammar mkExpr)
            "->type($string)"
            (newRef "main" (Node (typeExpr varStringExpr) Empty))
  , success "<= conflicts with <empty>"
            (grammar mkExpr)
            "<= 0"
            (newRef "main" (Node (leExpr varIntExpr (intExpr 0)) Empty))
  , success
    "unexpected space builtin treenode child"
    (grammar mkExpr)
    "A == \"F\""
    (newRef
      "main"
      (Node (eqExpr varStringExpr (stringExpr "A"))
            (Node (eqExpr varStringExpr (stringExpr "F")) Empty)
      )
    )
  , success "unexpected space after comment"
            (grammar mkExpr)
            "(* & */*spaces*/ )"
            (newRef "main" (And ZAny ZAny))
  , success
    "treenode with child builtin type"
    (grammar mkExpr)
    "A :: $string"
    (newRef
      "main"
      (Node (eqExpr varStringExpr (stringExpr "A"))
            (Node (typeExpr varStringExpr) Empty)
      )
    )
  , success "extra semicolon"
            (grammar mkExpr)
            "{*;*;}"
            (newRef "main" (Interleave ZAny ZAny))
  , success "user defined function"
            (grammar bothLibs)
            "->isPrime($int)"
            (newRef "main" (Node (isPrimeExpr varIntExpr) Empty))
